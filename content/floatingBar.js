...
function updateBarContent(state) {
    ...
    bar.querySelector('.pause-btn').addEventListener('click', (e) => {
        e.stopPropagation();
        chrome.runtime.sendMessage({ type: state.isPaused ? 'resumeFocus' : 'pauseFocus' });
    });

    bar.querySelector('.distraction-btn').addEventListener('click', (e) => {
        e.stopPropagation();
        chrome.runtime.sendMessage({ type: 'logDistraction' });
    });

    bar.querySelector('.dismiss-btn').addEventListener('click', (e) => {
        e.stopPropagation();
        dismissBar();
    });
}
...
function setupDrag() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        host.style.bottom = 'auto'; // Disable bottom alignment
        host.style.transform = 'none';
    });
